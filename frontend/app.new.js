...
async function handleStage1Submit() {
    ...
    if (!formData.vendorName || !formData.purchasePlace || !formData.pincode || !formData.purchaseDate || !formData.seedType) {
...
async function handleStage3Submit() {
    ...
    if (!batchNumber || !formData.packagingWorkerName || !formData.packagingDateTime) {
        showAlert('Please scan batch and fill all fields', 'error');
        return;
    }
